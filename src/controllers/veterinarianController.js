...
function getVeterinarian(req, res) {
    let document = req.cookies.document;

    if (!document) {
        return res.status(400).json({ error: 'Missing Veterinarian document cookie' });
    }

    const sql = `
        SELECT * FROM veterinarian WHERE document = ?
    `;

    connection.query(sql, [document], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ error: 'Error getting Veterinarian' });
        }
...
function putPasswordVeterinarian(req, res) {
